...
...SNIP...
```
### SSH Cracking with John

>Generating the hash using ssh2john.py
```bash
Attacker@xxx[/xxx]$ ssh2john.py SSH.private > ssh.hash
Attacker@xxx[/xxx]$ cat ssh.hash 

ssh.private:$sshng$0$8$1C258238FD2D6EB0$2352$f7b...SNIP...
```
>Cracking the Hash with John
```bash
Attacker@xxx[/xxx]$ john --wordlist=rockyou.txt ssh.hash
...
```
```bash
Attacker@xxx[/xxx]$ john ssh.hash --show
...
>Generating the hash using office2john.py
```bash
Attacker@xxx[/xxx]$ office2john.py Protected.docx > protected-docx.hash
Attacker@xxx[/xxx]$ cat protected-docx.hash

Protected.docx:$office$*2007*20*128*16*7240...SNIP...8a69cf1*98242f4da37d916305d8e2821360773b7edc481b
```
>Cracking the Hash with John
```bash
Attacker@xxx[/xxx]$ john --wordlist=rockyou.txt protected-docx.hash
...
>Generating the hash using pdf2john.py
```bash
Attacker@xxx[/xxx]$ pdf2john.py PDF.pdf > pdf.hash
Attacker@xxx[/xxx]$ cat pdf.hash 
...
>Cracking the Hash with John
```bash
Attacker@xxx[/xxx]$ john --wordlist=rockyou.txt pdf.hash
...
```
```bash
Attacker@xxx[/xxx]$ john pdf.hash --show
