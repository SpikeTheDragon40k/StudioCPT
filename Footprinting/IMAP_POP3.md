...
```bash
Attacker@xxx[/xxx]$ sudo nmap <IP> -sV -p110,143,993,995 -sC
...
| ssl-cert: Subject: commonName=mail1.inlanefreight.xxx/organizationName=Inlanefreight/stateOrProvinceName=California/countryName=US
...
```bash
Attacker@xxx[/xxx]$ curl -k 'imaps://<IP>' --user user:p4ssw0rd
...
```bash
Attacker@xxx[/xxx]$ curl -k 'imaps://<IP>' --user cry0l1t3:1234 -v

*   Trying <IP>:993...
* TCP_NODELAY set
* Connected to <IP> (<IP>) port 993 (#0)
...
* Server certificate:
*  subject: C=US; ST=California; L=Sacramento; O=Inlanefreight; OU=Customer Support; CN=mail1.inlanefreight.xxx; emailAddress=cry0l1t3@inlanefreight.xxx
*  start date: Sep 19 19:44:58 2021 GMT
*  expire date: Jul  4 19:44:58 2295 GMT
*  issuer: C=US; ST=California; L=Sacramento; O=Inlanefreight; OU=Customer Support; CN=mail1.inlanefreight.xxx; emailAddress=cry0l1t3@inlanefreight.xxx
*  SSL certificate verify result: self signed certificate (18), continuing anyway.
* TLSv1.3 (IN), TLS handshake, Newsession Ticket (4):
* TLSv1.3 (IN), TLS handshake, Newsession Ticket (4):
* old SSL session ID is stale, removing
< * OK [CAPABILITY IMAP4rev1 SASL-IR LOGIN-REFERRALS ID ENABLE IDLE LITERAL+ AUTH=PLAIN] xxx-Academy IMAP4 v.0.21.4
> A001 CAPABILITY
...
```bash
Attacker@xxx[/xxx]$ openssl s_client -connect <IP>:pop3s

CONNECTED(00000003)
Can t use SSL_get_servername
depth=0 C = US, ST = California, L = Sacramento, O = Inlanefreight, OU = Customer Support, CN = mail1.inlanefreight.xxx, emailAddress = cry0l1t3@inlanefreight.xxx
...
---
Certificate chain
 0 s:C = US, ST = California, L = Sacramento, O = Inlanefreight, OU = Customer Support, CN = mail1.inlanefreight.xxx, emailAddress = cry0l1t3@inlanefreight.xxx
...
---
read R BLOCK
+OK xxx-Academy POP3 Server
```

## OpenSSL - TLS Encrypted Interaction IMAP
```bash
Attacker@xxx[/xxx]$ openssl s_client -connect <IP>:imaps
