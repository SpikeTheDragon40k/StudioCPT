...
```bash
Adriano@htb[/htb]$ find /etc/ -name shadow
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name shadow 2>/dev/null
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name shadow 2>/dev/null > results.txt
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name shadow 2> stderr.txt 1> stdout.txt
```
...
```bash
Adriano@htb[/htb]$ cat < stdout.txt
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name passwd >> stdout.txt 2>/dev/null
```
...
```bash
Adriano@htb[/htb]$ cat << EOF > stream.txt
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd
```
...
```bash
Adriano@htb[/htb]$ find /etc/ -name *.conf 2>/dev/null | grep systemd | wc -l
```
